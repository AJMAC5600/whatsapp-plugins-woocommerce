(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(".template-selector").on("change", function () {
      ...
      const channelNumber = $selector
        .closest("table")
        .find(".channel-dropdown")
        .val();
        ...
        if (component && component.parameters) {
          ...
          component.parameters.forEach((param, index) => {
            ...
            const $label = $("<label>")
              .attr("for", inputId)
              .text(
                `${capitalizeFirstLetter(componentType)} Text ${index + 1}:`
              );
