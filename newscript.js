jQuery(document).ready(function ($) {
  ...
  const underline = jQuery(".underline");
  const heading = jQuery(".title");
  const otp = jQuery("#otp");
  const configurableSections = jQuery(".form-table").not(
    ":has(#api_key, #api_url)"
  );
  ...
  function generateTemplateInputs(templateData, section) {
    ...
    if (headerComponent) {
      headerComponent.parameters.forEach((param, index) => {
        const savedValue =
          localStorage.getItem(`${section}_header_${index}`) ||
          param.text ||
          "";
    ...
    if (bodyComponent) {
      bodyComponent.parameters.forEach((param, index) => {
        const savedValue =
          localStorage.getItem(`${section}_body_${index}`) || param.text || "";
        const $selectField = jQuery("<select>", {
          name: `body_${index}`,
        }).append(jQuery("<option>").val("").text("Select Variable"));
    ...
    if (buttonComponent) {
      buttonComponent.parameters.forEach((param, index) => {
        const savedValue =
          localStorage.getItem(`${section}_button_${index}`) ||
          param.text ||
          "";
  ...
  function saveInputValues(section) {
    const inputs = jQuery(
      `#header-inputs-${section} input, #body-inputs-${section} select, #button-content-${section} input`
    );
  ...
  jQuery("#save_changes").click(function (event) {
    ...
    sections.forEach((section) => {
      ...
      if (templateData) {
        const updatedData = recreateJsonWithUpdatedValues(
  ...
  function persistTemplateSelection() {
    sections.forEach((section) => {
      ...
      const savedTemplate = localStorage.getItem(
        `${section}_selected_template`
      );
      if (savedTemplate) {
        // First ensure category is selected
        const templateOption = $templateDropdown.find(
...
function fetchTemplatePayload(templateName, channelNumber, section) {
  ...
  jQuery.ajax({
    ...
    beforeSend: function (xhr) {
      ...
      jQuery(`#template-inputs-${section}`).html(
  ...
  function updateComponentParams(componentType) {
    ...
    if (component) {
      component.parameters.forEach((param, index) => {
        let inputField;
        // Update header and body to use select fields
